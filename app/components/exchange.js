...
class Blockchain extends React.Component {
  ...
  async setValue(e) {
    ...
    Exchange.methods.set(value).send();
    this._addToLog("Exchange.methods.set(value).send()");
  }

  getValue(e) {
    e.preventDefault();

    Exchange.methods.get().call().then(_value => this.setState({ valueGet: _value }));
    this._addToLog("Exchange.methods.get(console.log)");
  }
